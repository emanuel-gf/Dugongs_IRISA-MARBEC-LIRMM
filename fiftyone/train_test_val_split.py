...
def tag_train_test_split_seeded(train_size:float,
    ...
    for run in range(0,runs+1):
        ...
        logger.info(f"Seed number selected:{seed_number}")
        ...
        for island in islands_to_split:
            ...
            logger.success(f"Island {island}: Train={len(train_ids)}, Test={len(test_ids)}, Val={len(val_ids)}")
...
def argparse():
    ...
    parse.add_argument('--output-img-dir', type=str, help='Where to store the plot of Active Learning selection')
    parse.add_argument('--num-clusters', type=int, help='Num of clusters to cluster the dataset.')
    parse.add_argument('--ratio', type=float, help='Ratio of uniqueness over clusters. If 0.5 so 1:1, if 0.33 so 1:2, which means two images of cluster for each one image of uniqueness.')
        ...
        for partition in PARTITIONS:
            if partition > 0.5:
                ...
                final_al_indices = active_learning_pipeline(
                                        ...
                                        filename_image = f'AC_LR_seed{str(seed_number)}_partition_{str(int(partition*100))}_ratio{str(args.ratio)}.png',
